...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="f:tree" mode="tree-taxon-with-number">
    ...
    <xsl:if test="$taxon != ''">
      <xsl:call-template name="localized-label">
        <xsl:with-param name="label" select="$taxon" />
      </xsl:call-template>
  ...
  <xsl:template name="localized-label">
    <xsl:param name="label" />
    <xsl:choose>
      <xsl:when test="$label = 'Backlinks'">反向链接</xsl:when>
      <xsl:when test="$label = 'Related'">相关页面</xsl:when>
      <xsl:when test="$label = 'References'">参考文献</xsl:when>
      ...
      <xsl:when test="$label = 'Construction'">构造</xsl:when>
      <xsl:when test="$label = 'Convention'">约定</xsl:when>
      <xsl:when test="$label = 'Principle'">原则</xsl:when>
      <xsl:when test="$label = 'Outline'">提纲</xsl:when>
      <xsl:when test="$label = 'Chapter'">章节</xsl:when>
      <xsl:otherwise>
